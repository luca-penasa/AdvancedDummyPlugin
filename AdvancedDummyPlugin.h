...
//! Dummy qCC plugin
/** Replace the 'qBistacchi' string by your own plugin class name
    and then check 'qBistacchi.cpp' for more directions (you
...
**/
class AdvancedDummyPlugin : public QObject, public ccStdPluginInterface
{
	Q_OBJECT
	Q_INTERFACES(ccStdPluginInterface)
#ifdef CC_QT5
	//replace qDummy by the plugin name (IID should be unique - let's hope your plugin name is unique ;)
    Q_PLUGIN_METADATA(IID "cccorp.cloudcompare.plugin.AdvancedDummyPlugin")
#endif

public:

	//! Default constructor
    explicit AdvancedDummyPlugin(QObject* parent = 0);

	//inherited from ccPluginInterface
    virtual QString getName() const { return "AdvancedDummyPlugin"; }
...
protected:
	...
	/** You can add as many actions as you want in a plugin.
        ...
        Here is an exaple with two actions. One does somthing on a cloud,
        the other one on a mesh.
	**/
    QAction* m_doCreateCloud;
    QAction* m_doSomethingOnCloud;
};
